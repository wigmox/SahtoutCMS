...
// Optional: Verify TopG IP (uncomment if needed)
/*
$topg_ip = gethostbyname('monitor.topg.org');
$ip_request = $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0';
...
$p_resp  = isset($_GET['p_resp']) ? preg_replace('/[^A-Za-z0-9_-]/', '', $_GET['p_resp']) : null;
$user_ip = isset($_GET['ip']) ? filter_var($_GET['ip'], FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) : null;

if (!$p_resp || !$user_ip || strlen($p_resp) > 32) {
...
// Find TopG site entry
$callback_file_name = 'topg';
...
if (!$stmt || !$stmt->bind_param("s", $callback_file_name) || !$stmt->execute()) {
    http_response_code(500);
    header('Content-Type: application/json');
    echo json_encode(["status" => "error", "message" => "Vote site lookup failed."]);
    exit;
}
...
if ($result->num_rows === 0) {
    http_response_code(400);
    header('Content-Type: application/json');
    echo json_encode(["status" => "error", "message" => "Vote site not found."]);
    exit;
}
...
if (!$stmt || !$stmt->bind_param("ss", $p_resp, $p_resp) || !$stmt->execute()) {
    http_response_code(500);
    header('Content-Type: application/json');
...
if (!$can_vote) {
    http_response_code(403);
    header('Content-Type: application/json');
    echo json_encode(["status" => "error", "message" => "User is on cooldown."]);
    exit;
}
...
try {
    ...
    $stmt->bind_param("iisii", $internal_site_id, $user_id, $user_ip, $now, $reward_status);
    ...
    echo json_encode([
        "status" => "success",
        "message" => "Vote logged successfully for $p_resp (ID: $user_id).",
