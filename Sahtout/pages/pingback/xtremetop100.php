...
// Verify request source (XtremeTop100 IPs)
$xtt_ipv4 = '54.37.41.89';
$xtt_ipv6 = '2001:41d0:304:200::5096';
$ip_request = $_SERVER['REMOTE_ADDR'] ?? 'unknown';
if ($ip_request !== $xtt_ipv4 && $ip_request !== $xtt_ipv6) {
    http_response_code(403);
    header('Content-Type: application/json');
    echo json_encode(["status" => "error", "message" => "Invalid request source."]);
    exit;
}

// Get and validate parameters
$custom = isset($_GET['custom']) ? preg_replace('/[^A-Z0-9]/', '', $_GET['custom']) : null;
$voting_ip = isset($_GET['votingip']) ? filter_var($_GET['votingip'], FILTER_VALIDATE_IP) : null;

if (!$custom || !$voting_ip) {
    http_response_code(400);
    header('Content-Type: application/json');
    echo json_encode(["status" => "error", "message" => "Missing or invalid custom or votingip."]);
    exit;
}

if (strlen($custom) > 32 || strlen($custom) === 0) {
    http_response_code(400);
    header('Content-Type: application/json');
    echo json_encode(["status" => "error", "message" => "Invalid custom length."]);
    exit;
}

// Find XtremeTop100 site entry
$callback_file_name = 'xtremetop100';
...
if ($result->num_rows === 0) {
    http_response_code(400);
    header('Content-Type: application/json');
    echo json_encode(["status" => "error", "message" => "XtremeTop100 site not found in vote_sites."]);
    exit;
}
...
$stmt->bind_param("ss", $custom, $custom);
if (!$stmt->execute()) {
    http_response_code(500);
    header('Content-Type: application/json');
...
try {
    ...
    $stmt->bind_param("iisii", $internal_site_id, $user_id, $voting_ip, $now, $reward_status);
    if (!$stmt->execute()) {
    ...
    echo json_encode([
        "status" => "success",
        "message" => "Vote logged successfully for $custom (ID: $user_id).",
